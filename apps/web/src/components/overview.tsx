...
function ProjectCard({ proj, onOpen }: { proj: { id: string; label: string; color: string }; onOpen: () => void }) {
  ...
  async function handleDelete(e: React.MouseEvent) {
    e.stopPropagation();
    if (!confirm(t("overview.deleteConfirm", { label: proj.label }))) return;
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="font-mono text-caption text-muted-fg">{t("overview.card.runs", { count: stats.total })}</p>
        </div>
        ...
        <button
          ...
          aria-label={t("overview.deleteAria")}
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        <div>
          <p className="text-overline font-overline uppercase tracking-wide text-muted-fg">{t("overview.card.pass")}</p>
          <p className="text-[24px] font-title leading-none tracking-[-0.01em] text-success-fg">{stats.pass}</p>
        </div>
        <div>
          <p className="text-overline font-overline uppercase tracking-wide text-muted-fg">{t("overview.card.fail")}</p>
          <p className="text-[24px] font-title leading-none tracking-[-0.01em] text-error-fg">{stats.fail}</p>
        </div>
        <div>
          <p className="text-overline font-overline uppercase tracking-wide text-muted-fg">{t("overview.card.total")}</p>
      ...
      <div className="flex items-center justify-between border-t border-border pt-2 text-caption text-muted-fg">
        <span className="font-mono">{t("overview.card.coverage", { pct: coverage })}</span>
        <span className="flex items-center gap-1 text-muted-fg group-hover:text-foreground">
          ...
          {t("overview.card.viewRuns")} <span className="ghostly-project-card-arrow">→</span>
        </span>
      </div>
    </button>
  );
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-6 overflow-auto pb-4">
        <div className="flex shrink-0 flex-col gap-5">
          <div className="flex items-end justify-between gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-small text-muted-fg">
                {t("overview.subtitle", { projects: projects.length, runs: totals.runs, fail: totals.fail })}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-surface border border-border bg-card px-5 py-4">
              <p className="text-overline font-overline uppercase tracking-wide text-muted-fg">{t("overview.stats.projects")}</p>
              <p className="mt-1 font-serif text-[28px] font-title leading-none tracking-[-0.01em] text-foreground">{projects.length}</p>
            </div>
            <div className="rounded-surface border border-border bg-card px-5 py-4">
              <p className="text-overline font-overline uppercase tracking-wide text-muted-fg">{t("overview.stats.pass")}</p>
              <p className="mt-1 font-serif text-[28px] font-title leading-none tracking-[-0.01em] text-success-fg">{totals.pass}</p>
            </div>
            <div className="rounded-surface border border-border bg-card px-5 py-4">
              <p className="text-overline font-overline uppercase tracking-wide text-muted-fg">{t("overview.stats.fail")}</p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
          ...
          >
            ...
            <span className="text-small font-button">{t("overview.addProject")}</span>
          </button>
        </div>
      </div>
